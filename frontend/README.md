...
| パッケージマネージャ | npm                             |
| DB（予定）           | Postgres / Neon ＋ Prisma       |
| 認証（予定）         | Auth.js ＋ Google OAuth 2.0     |
...
| `external`   | アプリの外側との境界            | Prisma クライアント、Auth.js 設定、外部APIクライアント | ドメインの型やルール                    |
...
| `external` はドメインを知らない       | 接続先（Neon / Auth.js）を差し替えるとき、変更が `external` 内で収まる             |
...
| Prisma のクエリをどこに書くか  | 接続そのものは `external`、「どう取るか」は feature 側                                |
